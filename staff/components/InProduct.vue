<script>
import { getMyWaitingApi } from "@/api/erp/produce";
import { _chunk, _get, _groupBy, _isEmpty, _isEqual, _keys, _reverse } from "@/utils";
...
export default {
  ...
  methods: {
    getList(reset) {
      ...
      this.loading = true;
      getMyWaitingApi(this.queryList)
        .then(res => {
  ...
  computed: {
    GroupList() {
      const G = _groupBy(this.list, (item) => item.orderCode);
      return _keys(G).map((key) => {
        let list = G[key];
        list = list.map((c, i) => {
          const next = list[i + 1] || {};
          ...
          return {
            ...c,
            _nextStatus: next.status || c.status,
          };
        });
        const item = list[0] || {};
        ...
        return {
          label: key,
          customer: _get(item, "customer.name"),
          orderAddress: _get(item, "orderAddress"),
          isSuccess: list.every((e) => _isEqual(e.status, "FINISHED")),
          list: _chunk(list, 3)?.map((c, i) => {
            if (i % 2 === 0) {
              return c;
            } else {
              return _reverse(c);
    ...
    // 获取向下图标的状态
    getDownBtnStatus() {
      return (item, j) => _get(_get(item.list, `${j + 1}`)?.at(-1), "_nextStatus");
    },
  },
  created() {
    this.getList();
...
<template>
  ...
  >
    <view
      :class="{ is_success: item.isSuccess }"
      class="ko-in-product__item"
      v-for="(item, index) of GroupList"
      :key="index"
    >
      <view class="ko-in-product__header">
        <div class="ko-in-product__header--label">{{ item.label }}</div>
        ...
        <view v-if="item.orderAddress" style="margin-left: 10px; display:flex; align-items: center">
      ...
      <view class="ko-in-product__content">
        <block v-for="(G, j) of item.list" :key="j">
          <view class="ko-in-product__content--item" :class="j % 2 !== 0 ? 'end' : 'start'">
            <block v-for="(child, k) of G" :key="k">
              <view class="ko-in-product__cell" :class="child.status">
                ...
                <view>{{ getPricingMethod(child.pricingMethod) }}</view>
              ...
              >
                <uni-icons size="60" :type="j % 2 === 0? 'arrow-right' : 'arrow-left'" />
              </view>
            </block>
          </view>
          ...
          <view
            ...
            style="width: 100%"
            :class="[{ 'is-two': j % 2 !== 0 }, getDownBtnStatus(item, j), ]"
            v-if="item.list.length && j !== item.list.length - 1"
          >
            <view class="arrow-down__icon">
              <uni-icons size="60" type="arrow-down" />
            </view>
          </view>
        </block>
      </view>
