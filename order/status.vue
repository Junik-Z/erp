<script>
import mixins from "@/mixins/mixins";
import ProductCard from "@/components/ProductCard/ProductCard.vue";
import reLogin from "@/mixins/re-login";
import { getOrderStatusApi, getProduceInfoApi } from "@/api/erp/sale";
import UniSection from "@/uni_modules/uni-section/components/uni-section/uni-section.vue";
import StepList from "./components/StepList.vue";
import { _get } from "@/utils";

const T_CODE = /* "S2025033115025016924" || */ "";

export default {
  name: "status",
  data() {
    return {
      option: {},
      list: [],
      loading: true,
      info: {},
    };
  },
  mixins: [mixins, reLogin],
  onLoad(option) {
    this.option = option;
    console.log("获取到的登录数据", option);
  },
  components: {UniSection, StepList, ProductCard},
  methods: {
    // 获取订单详情
    getList() {
      this.loading = true;
      getProduceInfoApi({orderCode: T_CODE || this.Q_ID})
        .then(res => {
          this.info = res.data;
          console.log("订单信息", res.data, this.Q_ID);
          this.getStatus();
        })
        .finally(() => {
          setTimeout(() => {
            this.$nextTick(() => {
              this.loading = false;
            });
          }, 100);
        });
    },
    // 获取订单状态
    getStatus() {
      this.loading = true;
      getOrderStatusApi({orderCode: T_CODE || this.Q_ID})
        .then(res => {
          this.list = res.data;
          console.log("状态信息", res.data, this.Q_ID);
        })
        .finally(() => {
          setTimeout(() => {
            this.$nextTick(() => {
              this.loading = false;
            });
          }, 100);
        });
    },
  },
  computed: {
    // 获取订单地址
    getOrderAddress() {
      return child => _get(child, `0.orderAddress`) || "";
    },
    // 获取
    getCustomer() {
      return info => _get(info, `customer.name`) || "";
    },
  },
};
</script>

<template>
  <view class="ko-status">
    <view class="ko-status__title">
      <uni-icons style="margin-right: 6px" size="18" color="#1D4ED8" type="info" />
      <text>基础信息</text>
    </view>

    <view class="ko-status__basic" v-if="!loading">
      <view class="ko-status__basic-info">
        <view class="ko-status__label title" style="margin-bottom: 10px">
          {{ info.orderCode || "" }}

          <view class="ko-status__basic-info--status" :class="[info.status]">
            {{ PRODUCE_STATUS_ENUMS(info.status) }}
          </view>
        </view>

        <view class="ko-basic-table ko-basic-table__not-border" style="--ko-basic-table-grid-col: 50% 50%;">
          <view
            @click.stop="onCopyText(getCustomer(info))"
            class="ko-status__cell customer"
            v-if="getCustomer(info)"
          >
            <view class="icon">
              <uni-icons color="!0E7388" type="person" size="16" />
            </view>
            <view class="desc">
              <view class="tis">客户</view>
              <view>{{ getCustomer(info) }}</view>
            </view>
          </view>


          <view v-if="info.totalAmount >= 0" class="ko-status__cell total-amount">
            <view class="icon">¥</view>
            <view class="desc ko-basic-money">
              <view class="tis">订单金额</view>
              <view>{{ toYuan(info.totalAmount) }}</view>
            </view>
          </view>

          <view
            class="ko-status__cell order-phone"
            v-if="info.orderPhone"
            @click.stop="onCopyText(info.orderPhone)"
          >
            <view class="icon">
              <uni-icons color="#3B82F6" type="phone" size="16" />
            </view>
            <view class="desc">
              <view class="tis">联系电话</view>
              <view>{{ info.orderPhone }}</view>
            </view>
          </view>

          <view
            class="ko-status__cell order-address"
            v-if="info.orderAddress"
            @click.stop="onCopyText(info.orderAddress)"
          >
            <view class="icon">
              <uni-icons color="#22C55E" type="location" size="16" />
            </view>
            <view class="desc">
              <view class="tis">配送地址</view>
              <view>{{ info.orderAddress }}</view>
            </view>
          </view>
        </view>

      </view>
    </view>

    <block v-if="list.length">
      <view class="ko-status__title">
        <uni-icons style="margin-right: 6px" size="18" color="#1D4ED8" type="list" />
        <text>订单状态</text>
      </view>

      <block v-for="(item, index) of list" :key="index">
        <view class="ko-status__item">
          <StepList :list="item.amountsWithName" />
        </view>
      </block>

      <view class="ko-status__no-date" v-if="!list.length && !loading">
        <image
          mode="widthFix"
          class="ko-status__no-date--image"
          src="data:image/png;base64,"
        />
        <view>暂无订单状态</view>
      </view>
    </block>

    <block v-if="!isEmpty(info.productDetails)">
      <view class="ko-status__title">
        <uni-icons style="margin-right: 6px" size="18" color="#1D4ED8" type="color" />
        <text>成品</text>
      </view>

      <view class="ko-status__basic">
        <view class="ko-status__product" v-for="item of info.productDetails" :key="item.id">
          <view class="name">{{ item.name }}</view>
          <view class="ko-basic-table ko-basic-table__not-border" style="--ko-basic-table-grid-col: 50% 50%;">
            <view class="ko-status__cell total-amount">
              <view class="icon">¥</view>
              <view class="desc ko-basic-money">
                <view class="tis">单价</view>
                <view>{{ toYuan(item.price) }}</view>
              </view>
            </view>

            <view class="ko-status__cell product-quantity">
              <view class="icon">#</view>
              <view class="desc">
                <view class="tis">数量</view>
                <view>{{ item.productQuantity }}</view>
              </view>
            </view>
          </view>
        </view>
      </view>
    </block>

    <view class="ko-status__loading" v-if="loading">
      <uv-loading-icon size="40" />
    </view>
  </view>
</template>

<style scoped lang="scss">
.ko-status {
  width: 100%;
  padding-bottom: 50px;

  &__title {
    padding: 10px;
    font-weight: bold;
    color: rgb(29, 78, 216);
  }

  &__basic {
    padding: 0 10px 10px;
  }

  &__basic-info {
    padding: 10px;
    background-color: rgb(239 246 255 / 1);
    border: 1px solid rgb(219 234 254 / 1);
    border-radius: 6px;

    &--status {
      font-size: 12px;
      padding: 4px 10px;
    }
  }

  &__label {
    margin-bottom: 6px;
    display: flex;
    align-items: center;

    &.title {
      font-size: 18px;
      font-weight: bold;
      display: flex;
      align-items: center;
      justify-content: space-between;
    }
  }

  &__cell {
    margin-bottom: 10px;
    display: flex;
    align-items: center;

    .icon {
      width: 26px;
      height: 26px;
      border-radius: 50%;
      display: flex;
      align-items: center;
      justify-content: center;
      margin-right: 8px;
    }

    &.total-amount {
      .icon {
        background: rgb(254 226 226 / 1);
        color: #e43d33;
      }
    }

    &.customer {
      .icon {
        background: rgba(14, 115, 136, 0.3);
      }
    }

    &.order-phone {
      .icon {
        background: rgb(219 234 254 / 1);
      }
    }

    &.order-address {
      .icon {
        background: rgb(220 252 231 / 1);
      }
    }

    &.product-quantity {
      .icon {
        background: rgb(239 246 255 / 1);
        color: rgb(59 130 246 / 1);
      }
    }

    .desc {
      font-size: 15px;

      .tis {
        font-size: 10px;
        color: rgb(107 114 128 / 1);
      }
    }
  }

  &__item {
    padding: 0 10px 10px;
  }

  &__no-date {
    text-align: center;
    padding: 50px 0;
    color: #c7c9ce;
    font-size: 14px;

    &--image {
      width: 60vw;
      margin-bottom: 30px;
    }
  }

  &__loading {
    padding: 50px 0;
  }

  &__product {
    padding: 10px 10px 0;
    border: 1px solid #E5E7EB;
    border-radius: 6px;

    margin-bottom: 10px;

    --tw-shadow: 0 1px 2px 0 rgb(0 0 0 / 0.05);
    --tw-shadow-colored: 0 1px 2px 0 var(--tw-shadow-color);

    box-shadow: var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow);

    .name {
      font-size: 14px;
      font-weight: bold;
      margin-bottom: 10px;
    }
  }
}
</style>
