<template>
  ...
  >
    ...
    >
      <slot name="icon">
        <i
          class="iconfont icon-duigou uv-checkbox__icon-wrap__icon"
          :style="{fontSize: elIconSize + 'px', color: elIconColor}"
        ></i>
      </slot>
    </view>
    <view
...
export default {
  ...
  methods: {
    ...
    iconClickHandler(e) {
      this.preventEvent(e);
      // 如果整体被禁用，不允许被点击
      if (!this.elDisabled && !this.readonly) {
    ...
    labelClickHandler(e) {
      ...
      if (!this.elLabelDisabled && !this.elDisabled && !this.readonly) {
        this.setRadioCheckedStatus();
      }
    },
    emitEvent() {
...
@import '@/uni_modules/uv-ui-tools/libs/css/variable';
