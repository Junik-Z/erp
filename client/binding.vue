...
import { applyCustomerCodeApi } from "@/api/user";
...
export default {
  ...
  methods: {
    getList() {
      if (this.Q_ID) {
        this.loading = true;
        applyCustomerCodeApi({customerId: this.Q_ID})
          .then(() => {
            this.isSuccess = true;
          })
          .catch((res) => {
...
<template>
  <view class="ko-binding">
    <ZeroLoading
      type="gear"
      show-text
      :text="login_loading || loading ? '系统准备中...' : '客户绑定中，请稍后...'"
