  ...
  methods: {
    ...
    onSubmit() {
      this.$refs.FormRef.validate((valid) => {
        if (!valid) {
          const Func = this.isEdit ? editSupplierApi : addedSupplierApi;
  ...
  <view class="ko-client ko-basic-added-form">
    ...
    >
      <UniSection title="基础信息" type="line">
        <view style="padding: 10px;">
          <UniFormsItem label="供应商名称：" name="name">
    ...
    <BasicPopup :visible.sync="visible">
      <view class="ko-client__popup">
        ...
        <button class="ko-basic-button__card" style="margin: 0 40px 10px;" @click="addedFormContacts">保存</button>
      </view>
    </BasicPopup>
  </view>
</template>
