...
import { _isEmpty, _isEqual, _merge, getFileExtension, isImageType } from "@/utils";
  ...
  methods: {
        ...
        .then(e => {
          ...
          if (data.length > 0) {
            ...
            if (that.limit === 1) {
              ...
              that.$emit("input", [...this.value, ...value]);
            }
    ...
    onRemove(item) {
      if (this.limit === 1) {
        ...
        const name = item.tempFile?.name;
        const val = this.value.filter(v => !_isEqual(v, name));
        this.$emit("input", val);
      }
    },
