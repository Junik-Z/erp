...
import { _deepCopy, _isEmpty, _isEqual, showToast } from "@/utils";
...
export default {
  ...
  methods: {
    ...
    addedTicket(node) {
      this.visible = true;
      this.form = _isEmpty(node) ? _deepCopy(this.$options.data().form) : _deepCopy(node);
      ...
      this.$refs.FormRef.validate((valid) => {
        if (!valid) {
          const Func = this.isEdit
            ...
            : this.isRefund ? addedPaidOrderApi : addedReturnedOrderApi;
          ...
          <BasicCard :spacing="10" v-for="(item, index) of list" :key="index">
            <view class="ko-ticket__item">
              ...
              <view class="ko-ticket__item--info">
                ...
                <view class="ko-ticket__item--button" v-if="step === 0">
