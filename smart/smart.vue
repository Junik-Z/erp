...
<template>
  <view class="ko-smart" :style="[rootStyles]">
    <image class="ko-smart__robot" :src="getShopImage" mode="heightFix" />
    ...
    <view class="ko-smart__wrap">
      ...
      <view class="ko-smart__footer">
        <view class="ko-smart__mic" :class="{'is-cancel': isCancel}">
          <!-- 打字聊天 -->
          <block v-if="isMsg">
...
.ko-smart {
  ...
  &__effect {
    ...
    &::before {
      ...
      left: 50%;
      transform: translatex(-50%);
      bottom: $--diffusion-pos;
      width: var(--vol-size, 10px);
