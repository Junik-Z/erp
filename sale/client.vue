...
export default {
  ...
  methods: {
    ...
    onJump(row) {
      this.noRefresh = true;
      uni.navigateTo({url: `${PageEnums.saleNewClient}?id=${row.id}`});
    },
    ...
    onTrigger(event) {
      if ("uni") {
        this.noRefresh = true;
        uni.navigateTo({url: PageEnums.saleNewClient});
        return false;
      }
    ...
    onJumpInfo(item) {
      if (this.isPerm("FINANCE_RECEIVABLE_CHECK") || this.isPerm("FINANCE_PAYABLE_CHECK")) {
        this.noRefresh = true;
        uni.navigateTo({
          url: PageEnums.financeCheck + `?id=${item.id}&customer_type=sale&FORM=sale`,
        });
      }
    },
